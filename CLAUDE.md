...
`sales`（注文まわり）、`event`（お誘い。→ `docs/domain/invite/`）、お店（→ `docs/domain/lunch/`。記号は naming で確定）。
**参照は `sales` → `common`、`event` → `udb`、お店 → `udb` の一方向。`udb` は参照される側で、逆流させない。
`event` とお店は互いに参照しない（結合ゼロ）。**
