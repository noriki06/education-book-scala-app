...
業務が使う日本語（スタンプ、スタンプカード、種別）はそのままで、英語名だけが変わる（→ 02_analysis.md）。
...
| スタンプカード | 種別が定める数のスタンプを貯める1枚。有効期限を持ち、特典交換の単位になる | 台紙、シート |
| スタンプカードの種別 | カードの有効期間・必要スタンプ数・特典の内容を定めるマスタ（→ naming.md） | キャンペーン、プラン |
| 配布期間 | 種別が持つ。この期間中に会員が最初のスタンプを得た場合だけ、その種別のカードができる | 開催期間（会員への案内文言としては使うが、モデルの呼称は配布期間で統一する） |
| 有効期限 | スタンプカードが持つ、そのカードが使えなくなる日。できた日（＝最初のスタンプを得た日）＋種別が定める有効期間で確定する。以後延長しないが、期間限定の種別に参加すると開催期間の長さだけ加算される | 失効予定日、有効期間 |
| 使用状態 | 特典交換に使ったかどうかを表す。使用済み／未使用の2値 | 消化、消費済み |
| 有効スタンプ | 期限内かつ未使用のスタンプカードに属するスタンプ | 残スタンプ、使えるスタンプ |
| 特典 | 必要スタンプ数を満たしたときに交換できるもの。全額無料／半額／金額割引（円）のいずれか1つを種別が定める | クーポン、サービス券 |

## ER 図
![スタンプまわりのER図。左に既存の会員・店舗・注文・商品が並び、右上に今回追加するスタンプカードの種別（マスタ）、右下にスタンプカードとスタンプがある。会員とスタンプカードは1対多、スタンプカードとスタンプは1対多、種別とスタンプカードは1対多、注文とスタンプは1対1](./img/er-stamp.svg)

⚠️ 上図は命名見直し前のもの。`Reward` / `RewardCard` / `RewardCardStamp` のクラス名で描き直す必要がある（別途対応）。
...
## 「使用状態」を区分値にする理由（考え直した点）

以前の判断（旧2階層時点）は逆だった。**「使用済み」は区分値にしない、`usedAt` の有無で表す。**
日時は問い合わせ対応で必要になるため結局持つことになり、区分値を足すと同じ事実が2か所に現れる、という理由だった。

今回、管理側から「特典の利用実績を集計したい」という要求が出たことで `state` を追加した。
`usedAt IS NOT NULL` でも同じ集計はできるため、厳密には `state` が無くても要求は満たせる。
それでも追加したのは、**集計・一覧のための列（`state`）と、個々の記録のための列（`usedOrderId` / `usedAt`）の役割を分けたい**という判断のため。
代償は当時警告したとおり——同じ事実を2か所で持つことになり、片方だけ書き換える書き忘れが起こり得る。
この代償を消せないので、更新経路を1つの操作関数に絞ることで対応する（→ 型では守れない決めごと）。

**「期限切れ」は区分値にしない（変更なし）。** 保存すると、日付が変わった瞬間に嘘になる。
...
`udb` にも置けません——`Order.Id` を参照する側のため、`udb → sales` の逆流が生まれます。
...
**旧設計（2階層時点）は延長する側だった。** 「一律延長」の要件を素直に読めば、保有するカードすべてが対象になる。
しかしそのままだと10個揃えた会員は注文を続けるかぎり特典が永久に消えないため、満杯後だけ延長を止める、
という複雑な例外を作っていた。

種別（マスタ）を導入したことで前提が変わった。種別ごとに「この期間だけ有効」という値を明確に定義したかった
（→ 01_requirements.md の追加要求）ため、注文ごとに動く延長ロジックはむしろ邪魔になる。
固定した結果、「満杯後は延長しない」という以前の例外も丸ごと不要になった——固定なのだから、
満杯かどうかで扱いを分ける理由がそもそも無い。

例外として期間限定の種別への参加だけは延長する。これは注文のたびに自動で起こるものではなく、
...
**旧設計（2階層時点）は日時の有無だけだった。** 「いつ交換したか」は問い合わせ対応で必要なので日時は結局持つ、
日時を持つなら区分値は同じ事実の言い換えにすぎない、という理由だった。

今回、管理側から「特典の利用実績を集計したい」という要求が明示的に出たことで判断を変えた。
`usedAt IS NOT NULL` でも集計自体は可能だが、集計・一覧表示のための列と、
個々の記録（いつ・どの注文で）のための列の役割を分けたいという要求を優先した。

代償は当時警告したとおりのもの——同じ事実を2か所に持つことになり、
「`state` は使用済みなのに `usedAt` が空」という矛盾が作れてしまう。
この矛盾を型で防ぐことはできないため、更新を「使用済みにする」「差し戻す」の2つの操作関数に絞り、
...
`state` を追加した後もこの理由は変わらないため、`usedOrderId` は引き続き持つ。
...
#### 種別をマスタ（エンティティ）にするか、`RewardCard` の属性として複製するか
...
当初案（`StampCardType` / `StampCard` / `Stamp`）は、`Stamp` という語根が最上位と中間の両方に現れ、
深い階層ほど名前が短くなる、`Cart` 系列とは逆向きの並びだった（⑧が4点、→ naming.md）。
`Stamp` を末端の1語に限定し、上位2階層を別の語根から組み立てることで、深いほど名前が長くなる並びに直した。

代償は、業務が「スタンプカード」と呼ぶもののクラス名が `RewardCard` になり、直訳ではなくなったこと。
ユビキタス言語の対応表（→ 02_analysis.md）で会話とコードをつなぐ。
